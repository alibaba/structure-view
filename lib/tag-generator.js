...
export default class TagGenerator {
  ...
  async generate() {
    ...
    if (tags.scriptNode && tags.scriptNode.length) await this.inlineScriptHandler(tags.scriptNode);
    return tags;
  }

  async inlineScriptHandler(nodes) {
    let parser = require('./tag-generators/javascript');
    nodes.forEach(async i => {
      i.child = (await parser.parseFile({ content: i.content })).tree;
      if (Object.keys(i.child).length) this.fixLineno(i);
    });
  }
