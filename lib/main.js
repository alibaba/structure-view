...
export default {
    ...
    switch (stat) {
        ...
        if (editors.length < 1 ||
            (editors.length === 1 && !editors[0].getPath()) ) {
            atom.confirm({ message: 'WARN', detailedMessage: 'No file is opened!', buttons: { Close: function () { return; } } });
            return;
        }
        ...
        // Force recalculate in order to get dock title
